...
AUCdata2 <- data.frame(`True Positive Rate`=c(unlist(attributes(prf)$y.values),unlist(attributes(prf2)$y.values)),
                       ...
                       type=c(rep("Initial Logistic",length(unlist(attributes(prf)$y.values))),
                              rep("Support Vector Machine",length(unlist(attributes(prf2)$y.values)))))

ppi <- 250
Sinfo = 40
png("SVM.png",width=20*ppi,height=12*ppi,res=ppi)
...
aucArrayPdat <- data.frame(values=aucArray, trial=1:100)

ppi <- 250
Sinfo = 40
png("CVlogit.png",width=20*ppi,height=12*ppi,res=ppi)

ggplot(aucArrayPdat,aes(x=trial,y=values)) +
  geom_line(size=2) +
...
for(i in 1:100){
  ...
  testIds <- runif(nrow(Xdata)*0.1,1,nrow(Xdata))
  pr <- prediction(predict(svmfit,newdata = Xdata[testIds,-1],type="response"), Xdata[testIds,'approved'])
